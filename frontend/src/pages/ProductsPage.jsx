...
import { Check, ArrowRight, Shield, Zap, Eye, Globe, BarChart3, Users } from 'lucide-react';
...
const ProductCard = ({ product, index }) => {
  const icons = [Shield, Zap, Eye, Globe, BarChart3, Users];
    ...
    <Link to={product.href} className="group">
      <div className="bg-white rounded-2xl p-8 border border-gray-100 hover:border-[#2563EB] hover:shadow-xl transition-all duration-300">
        ...
        <div className="flex items-center gap-2 mb-2">
          <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#2563EB] transition-colors">{product.name}</h3>
          {product.isNew && (
          ...
          <div className="max-w-7xl mx-auto px-4">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {productsMenu.map((product, idx) => (
                <ProductCard key={`product-${idx}`} product={product} index={idx} />
              ))}
            </div>
          </div>
        </section>
